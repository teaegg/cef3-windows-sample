...
class OSRBrowserProvider {
 public:
  virtual CefRefPtr<CefBrowser> GetBrowser(HWND hWnd) =0;
...
class OSRWindow : public ClientHandler::RenderHandler {
 public:
  ...
  // object.
  static CefRefPtr<OSRWindow> Create(HINSTANCE hInst,
  ...
  HWND CreateWidget(HWND hWndParent, const RECT& rect, LPCTSTR windowName, LPCTSTR className);
 ...
 private:
  OSRWindow(HINSTANCE hInst, OSRBrowserProvider* browser_provider, bool transparent);
  virtual ~OSRWindow();

  void Render();
  void EnableGL();
  void DisableGL();
  void OnDestroyed(HWND hWnd);
